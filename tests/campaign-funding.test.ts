...
const goal = 50000;
const hash = "70573301787ea4db801ca44a7b9ecd28443jjk11";
...
function addCampaign(blockDuration: number = 50) {
  const { result: addCampaignResult } = simnet.callPublicFn(
    "campaign-funding",
    "add-campaign",
    [
      Cl.stringUtf8(title),
      Cl.uint(goal),
      Cl.uint(blockDuration),
      Cl.stringUtf8(hash),
    ],
    address1
  );
  ...
  const { result: getCampaignResult } = simnet.callReadOnlyFn(
    "campaign-funding",
    "get-campaign",
    [Cl.uint(newCampaignId)],
    address2
  );

  const blockHeightAtCreation = simnet.blockHeight;
  const expectedCampaign = Cl.tuple(
    buildCampaignData(blockHeightAtCreation, blockDuration)
  );
...
describe("add-campaign", () => {
  it("adds a campaign and initializes its contribution data", () => {
    ...
    const { result: getCampaignFundingTotalsResult } = simnet.callReadOnlyFn(
      "campaign-funding",
...
describe("update-campaign-data", () => {
  it("updates campaign data", () => {
    ...
    const newHash = "848573nfk38fj4kl3r4565234567kkll34311100";

    const { result: updateCampaignResult } = simnet.callPublicFn(
      "campaign-funding",
      "update-campaign-data",
      [Cl.uint(newCampaignId), Cl.stringUtf8(newTitle), Cl.stringUtf8(newHash)],
      address1
    );
    ...
    const expectedCampaign = Cl.tuple({
      ...
      "data-hash": Cl.stringUtf8(newHash),
    });
    expect(getCampaignResult.value).toEqual(expectedCampaign);
  });
...
describe("contribute-to-campaign", () => {
  it("collects a contribution from caller and updates funding data for the campaign", () => {
    ...
    const { result: contributeResult } = simnet.callPublicFn(
      ...
      [Cl.uint(newCampaignId), Cl.uint(amountContributed)],
      address1
    );
    ...
    const { result: contributionResult } = simnet.callReadOnlyFn(
      "campaign-funding",
  ...
  it("fails and does not collect stx if the campaign has already ended", () => {
    ...
    const { result: contributeResult } = simnet.callPublicFn(
      "campaign-funding",
      "contribute-to-campaign",
      [Cl.uint(newCampaignId), Cl.uint(1000)],
      address1
    );
  ...
  it("sums contributions by the same address, and keeps data updated for contributions from different addresses", () => {
    ...
    const { result: contributeResult1 } = simnet.callPublicFn(
      "campaign-funding",
      "contribute-to-campaign",
      [Cl.uint(newCampaignId), Cl.uint(contributionAmount1)],
      address1
    );
    expect(contributeResult1).toBeOk(Cl.bool(true));
    const { result: contributeResult2 } = simnet.callPublicFn(
      "campaign-funding",
      "contribute-to-campaign",
      [Cl.uint(newCampaignId), Cl.uint(contributionAmount2)],
      address1
    );
    expect(contributeResult2).toBeOk(Cl.bool(true));
    const { result: contributeResult3 } = simnet.callPublicFn(
      "campaign-funding",
      "contribute-to-campaign",
      [Cl.uint(newCampaignId), Cl.uint(contributionAmount3)],
      address2
    );
    ...
    expect(newStxBalanceAccount1).toBe(
      startingStxBalanceAccount1 - BigInt(contributionTotalAccount1)
    );
    expect(newStxBalanceAccount2).toBe(
      startingStxBalanceAccount2 - BigInt(contributionAmount3)
    );
    ...
    const { result: contributionResultAccount1 } = simnet.callReadOnlyFn(
      "campaign-funding",
      "get-contribution-info",
      [Cl.uint(newCampaignId)],
      address1
    );
    expect(contributionResultAccount1.value).toEqual(
      Cl.tuple({
        amount: Cl.uint(contributionTotalAccount1),
        "is-refunded": Cl.bool(false),
      })
    );
    const { result: contributionResultAccount2 } = simnet.callReadOnlyFn(
      "campaign-funding",
      "get-contribution-info",
      [Cl.uint(newCampaignId)],
      address2
    );
    expect(contributionResultAccount2.value).toEqual(
      Cl.tuple({
...
describe("refund-contribution", () => {
  it("refunds a contribution back to the given contributor", () => {
    ...
    const { result: refundResult } = simnet.callPublicFn(
      "campaign-funding",
      "refund-contribution",
      [Cl.uint(newCampaignId), Cl.principal(address1)],
      address2
    );
  ...
  it("cannot refund the same contribution more than once", () => {
    ...
    const { result: refundResult2 } = simnet.callPublicFn(
      "campaign-funding",
      "refund-contribution",
      [Cl.uint(newCampaignId), Cl.principal(address1)],
      address2
...
describe("fund-campaign", () => {
  it("sends funds to the campaign owner", () => {
    ...
    expect(getCurrentStxBalance(address1)).toBe(
      startingStxBalanceAccount1 + BigInt(amountContributed)
    );
    expect(getCurrentStxBalance(address2)).toBe(
      startingStxBalanceAccount2 - BigInt(amountContributed)
    );
  ...
  it("cannot fund the same campaign more than once", () => {
    ...
    const { result: fundResult2 } = simnet.callPublicFn(
      "campaign-funding",
      "fund-campaign",
      [Cl.uint(newCampaignId)],
      address2
